		...
		<tr>
			...
			<td valign="top"><spform:form class="pure-form" method="post"
					...
					<fieldset>
						<legend>
							<b>Employee form</b> | <a
								href="${ pageContext.request.contextPath }/mvc/session15/job/">Job
								form</a>
						</legend>
			...
			<td valign="top" colspan="4">
				<form class="pure-form">
					<fieldset>
						<legend>
							Employee List | <a
								href="${ pageContext.request.contextPath }/mvc/session15/employee/page/-1">全部查詢</a>
							| 分頁查詢：
							...
							<tbody>
								<c:forEach var="emp" items="${ employees }">
									<tr>
										...
										<td>${ emp.ename }</td>
										<td>${ emp.salary }</td>
										<td><c:forEach var="job" items="${ emp.jobs }">
												<a title="${ job.jname }"
