		...
		<tr>
			...
			<td valign="top"><spform:form class="pure-form" method="post"
					...
					<fieldset>
						<legend>
							<a
								href="${ pageContext.request.contextPath }/mvc/session15/employee/">Employee
								form</a> | <b>Job form</b>
						</legend>
						<input type="hidden" name="_method" id="_method"
							value="${ _method }" /> 編號：
						<spform:input path="jid" readonly="true" />
						<p />
						名稱：
						<spform:input path="jname" />
						<br />
						<spform:errors path="jname" cssClass="error" />
						<p />
						員工：
						<spform:select path="eid" items="${ employees }" itemLabel="ename"
							itemValue="eid" />
						<p />

						<button type="submit" class="pure-button pure-button-primary"
							${ _method eq 'POST'?'':'disabled' }>新增</button>
						<button type="submit" class="pure-button pure-button-primary"
							${ _method eq 'PUT'?'':'disabled' }>修改</button>
						<button type="button" class="pure-button pure-button-primary"
							${ _method eq 'PUT'?'':'disabled' }
							onclick="changeMethod('DELETE');">刪除</button>
						<p />
			...
			<td valign="top" colspan="2">
				<form class="pure-form">
					<fieldset>
						<legend>
							Job List | <a
								href="${ pageContext.request.contextPath }/mvc/session15/job/page/-1">全部查詢</a>
							| 分頁查詢：
							<c:forEach var="num" begin="1" end="${ pageCount }">
								<a
									style="${ sessionScope.num == num ? 'text-decoration: none':''  }"
								...
								<c:forEach var="job" items="${ jobs }">
									<tr>
										<td><a
											...
											title="按我一下可以修改"> ${ job.jid } </a></td>
										<td>${ job.jname }</td>
										<td>${ job.employee.eid }</td>
										<td><a
											href="${ pageContext.request.contextPath }/mvc/session15/employee/${ job.employee.eid }"
											title="按我一下可以修改員工資料"> ${ job.employee.ename } </a></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
